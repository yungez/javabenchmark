...
// 2. vm
resource.createOrGetAzureVM = function (resourceGroupName, vmName, region, size, callback) {
    return azureVM.createOrGetVirtualMachine(resourceGroupName, vmName, region, size, callback);
}
...
// aws
// 1. elasticBeanStalk
resource.createOrGetAWSElasticBeanStalk = function () {
...
resource.startAWSEC2Instance = function (instanceId, action, callback) {
    return awsEC2.nanipulateEC2Instance(instanceId, 'ON', callback);
}

resource.stopAWSEC2Instance = function (instanceId, action, callback) {
    return awsEC2.nanipulateEC2Instance(instanceId, 'OFF', callback);
}

resource.terminateAWSEC2Instance = function (instanceId, callback) {
    return awsEC2.terminateEC2Instance(instanceId, callback);
}
