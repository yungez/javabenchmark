...
var webSiteManagementClient = new azureArmWebsite(credentials, subscriptionId);
...
function deleteWebApp(resourceGroupName, webappName, callback) {
    webSiteManagementClient.webApps.deleteMethod(resourceGroupName, webappName, function (err, result) {
        if (err) {
            ...
            console.log(`site ${name} deleting successful.`);
            return callback(err, result);
        }
    });
}
...
function stopWebApp(resourceGroupName, name, callback) {
    webSiteManagementClient.stop(resourceGroupName, name, null, function (err, result) {
        if (err) {
            console.error(err);
            return callback(err, result);
        } else {
            console.log(`site ${name} stopping successful.`);
            return callback(err, result);
        }
    });
}
...
function startWebApp(resourceGroupName, name, callback) {
    webSiteManagementClient.start(resourceGroupName, name, null, function (err, result) {
        if (err) {
            console.error(err);
            return callback(err, result);
        } else {
            console.log(`site ${name} starting successful.`);
            return callback(err, result);
        }
    });
}
